...
function Format-PesterTest {
    ...
    process {
        ...
        if ($TestResult.Passed) {
            Write-Output 'Passed' -NoNewline -ForegroundColor Green
        }
        else {
            Write-Output 'FAILED' -NoNewline -ForegroundColor Red
        }

        Write-Output ( '] {0}: {1}: {2}' -f $TestResult.Describe, $TestResult.Context, $TestResult.Name )

        if ($TestResult.Passed -eq $false) {
            Write-Output $TestResult.FailureMessage -ForegroundColor Yellow
            if ($TestResult.StackTrace) {
                Write-Output $TestResult.StackTrace.ToString() -ForegroundColor Yellow
            }
        }
    }
}
...
if ($failedTests) {
    Write-Output 'Please review these failed tests:'
